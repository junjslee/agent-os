...
최근 학계에서는 에이전트가 맥락에서 실제로 아는 것, 당신이 의도한 것, 시스템의 실제 상태 사이의 누적된 간격을 **Epistemic Drift (인식론적 표류)** 라고 부른다. `episteme`는 에이전트가 행동하기 전에 *무엇 · 왜 · 어떻게* 를 구조적으로 추론하도록 요구함으로써 그 간격을 닫는다.
