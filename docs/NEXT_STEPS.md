...
9. **Logo character mark (proposal in flight, 2026-04-22, soak-safe).** Three candidates live at `docs/assets/logo-candidates/` — A (wizard-sage), B (sage with summoned dragonling), C (dragon sigil) — each composing a pixel character mark in deep indigo (`#1A1740` / `#2E2A6B` / `#6B5FB8` / `#D4CEEF` + `#F8E7A3` warm accent) with the existing wordmark. Operator to pick direction; candidates README at the same path documents the pick-and-archive workflow. Post-pick follow-ups: derive dark-mode variant, wire into `README.md` `<picture>` block (currently at `docs/assets/logo-{light,dark}.svg` — wordmark-only today), wire into `web/src/app/` Next.js nav/hero, 24×24 favicon survival test, optional CLI half-block render (~14 cells wide, truecolor ANSI) for `episteme init` banner. Scope: `docs/assets/` + `README.md` + `web/src/app/`. Does NOT touch `core/hooks/` or `kernel/*`. Soak-safe.
