...
**So-What Now? — 2026-04-23 late-session pivot.** The 7-day soak calendar wait has been **halted in favor of immediate Day-2 evidence-based gate-grading** (operator decision, 2026-04-23). Engineering gates all green (`pytest 587/587`, `episteme doctor` 10 ok / 6 info / 0 warn / 0 fail, `episteme review --list` empty, `kernel/MANIFEST.sha256` in sync). Next action: produce per-gate verdict (clear-pass / clear-fail / ambiguous-thin-data) on Gates 21-28 using existing episodic records + reasoning-surfaces + framework chain data; if ≥ 4 of 8 pass clearly, open the v1.0 GA window; if < 4 or majority ambiguous, extend soak evidence-based (not calendar-based) until the weakest gates accumulate enough data. **Day-2 Gate Grading summary appears in § "Day-2 Gate Grading" below this section once produced this session.** In parallel, three soak-safe workstreams being executed this session: (a) **Event 34 — Korean README (`README.ko.md` + `/readme/ko` route) queued + shipping**; (b) **Event 35 — `release-please` automation (closes NEXT_STEPS line 212 version-string consistency gap) queued + shipping**; (c) **v1.0.1 · Chain Hygiene CP update — swap cron → event-driven via `Stop` hook async handler, AND bundle gbrain-inspired zero-LLM entity extraction into the same CP (one coherent CP instead of two — maintenance loop is the natural host for entity extraction).**

Soak-safety intact: all four workstreams touch zero `core/hooks/` or `kernel/*` code — distribution / web / docs / CI surfaces only. Cognitive-adoption gate measurement continues against the tagged `v1.0.0-rc1` state uncontaminated.

**All today's 8 commits (plus 2 handoff/meta commits) pushed to `origin/master`; working tree clean; next session can resume cold.**

### Day-2 Gate Grading (produced 2026-04-23, Day 2 of 7-day soak)
...
8. **v1.0.1 · Chain Hygiene CP — deferred-discovery write-side dedup + resolution-record type (post-soak; `core/hooks/` + framework-chain schema; queued 2026-04-23).** **DO NOT START DURING SOAK** — touches `core/hooks/_blueprint_d.py` / `core/hooks/_chain.py` and the framework chain schema, both frozen until ~2026-04-29. Queued because this session surfaced that `~/.episteme/framework/deferred_discoveries.jsonl` holds **1,201 records but only 32 unique descriptions** (first-80-char match); top 4 descriptions are each duplicated exactly **105 times**, with a 5th at 82 — a systematic re-logging signature, not scattered re-discovery. Two distinct plumbing gaps fuel the inflation:

   - **Gap 1 · Write-side dedup missing.** When a reasoning-surface's `deferred_discoveries[]` array writes to the chain (via Blueprint D validator → `_chain.append`), no check runs against already-chained content. Every Blueprint D firing whose source surface carries a persisted discovery entry appends the full array verbatim. Fix shape: content-hash dedup on append. Two candidate implementations — (a) hash `payload.description + payload.flaw_classification + payload.observable` → skip append if match found within a recency window (N=30 days candidate; cheap O(n) scan on the chain tail); (b) maintain a side-index (`~/.episteme/framework/deferred_discoveries.seen.json`) of content-hash → most-recent-entry-hash, short-circuit append on hit. (a) preserves pure chain-as-source-of-truth; (b) needs a verify-from-chain bootstrap + side-index invalidation rules. Pick (a) unless profiling shows chain-tail scan is too slow at scale. Preserves Pillar 2 append-only — dedup is a write-time admission check, not a post-hoc chain mutation.

   - **Gap 2 · No resolution-record type.** Hash chain is append-only by design (Pillar 2), so `payload.status: "pending"` on a prior entry never flips to `resolved` in-place. Nothing in the current schema says "entry X was closed by work Y." Fix shape: new chain payload type `deferred_discovery_resolution` carrying: `resolves_entry_hash` (target entry's `entry_hash`), `resolver_commit_sha` (git SHA of the commit that closed it), `resolution_verdict` (enum: `fixed` / `superseded` / `wont-fix` / `cannot-reproduce`), `resolver_rationale` (one-paragraph why), `logged_at` (ISO-8601). Chain verification extended to cross-reference — if a `deferred_discovery` entry has a subsequent `deferred_discovery_resolution` entry that references its hash, the pending-count query filters it out. `episteme guide --deferred` + SessionStart hook digest both gain `unresolved_only` semantics (current shape: `count of chain records with payload.type == 'deferred_discovery'` → post-fix: `that count MINUS count of records with a matching resolution entry`).
   ...
   **Scope bound (explicit, to prevent post-soak scope creep):**
   ...
   - **In scope.** Write-side dedup in the deferred_discoveries write path; new `deferred_discoveries_resolution` chain payload type; `_chain.verify()` extension to cross-link resolution → target; `episteme guide --deferred` filter on unresolved-only; SessionStart digest filter; tests for dedup-on-append + resolution-record-append + pending-count-with-resolutions; one-time triage pass on the 32 existing unique descriptions in the live chain (file `deferred_discovery_resolution` records for the ones genuinely closed, mark the rest `wont-fix` or leave pending as intended).
   - **Out of scope.** Restructuring the `deferred_discoveries[]` field on the reasoning-surface schema itself (schema-evolution candidate for Phase C). Restructuring episodic or protocols streams (this CP is surgical to the deferred_discoveries stream). Backfill-resolving the full 1,201-record set — the triage targets the 32 unique descriptions only; historical duplicates stay in the chain as append-only history.
